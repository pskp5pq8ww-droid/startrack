...
const state = {
  data: {
    ...
    sickLeaves: [],
    routes: [],
    settings: { depotName: 'BNPF Depot', systemName: 'DepotOS', cutOffTime: '18:30', defaultBreakMinutes: 60 },
  },
  currentUser: null,
  view: 'home',
  intent: '',
...
// ── Brand identity (client-side switch, persisted per device) ───────────────
const BRANDS = {
  depotos: { id: 'depotos', name: 'DepotOS', logo: './depotos-logo.svg' },
  nktransport: { id: 'nktransport', name: 'NK Transport', logo: './nktransport-logo.svg' },
}
const getBrandId = () => { try { return localStorage.getItem('nk.brand') || 'depotos' } catch (e) { return 'depotos' } }
const setBrandId = (id) => { try { localStorage.setItem('nk.brand', id) } catch (e) {} }
const brand = () => BRANDS[getBrandId()] || BRANDS.depotos
const applyBrand = () => {
  document.documentElement.dataset.brand = brand().id
  document.title = `${brand().name} KPI`
}
const brandSwitch = () => {
  const active = getBrandId()
  const seg = (b) => `<button class="brand-switch-seg ${active === b.id ? 'active' : ''}" data-action="set-brand" data-id="${b.id}" type="button">${h(b.name)}</button>`
  return `<div class="brand-switch" role="group" aria-label="Brand identity switch">${seg(BRANDS.depotos)}${seg(BRANDS.nktransport)}</div>`
}
applyBrand()
...
const header = () => {
  ...
  return `
    <header class="topbar">
      <button class="brand" data-action="home" aria-label="${h(brand().name)} home">
        <img src="${brand().logo}" alt="${h(brand().name)}" class="brand-logo" />
...
const homeView = () => `
  <main class="home-screen">
    <button class="logo-admin-btn" data-action="admin-access" aria-label="${h(brand().name)} staff login">
      <img src="${brand().logo}" alt="${h(brand().name)}" />
    </button>
      ...
      <section class="login-panel">
        <form class="form" data-form="login">
          <div class="view-title">
            <p class="eyebrow">${brand().name} · ${roleLabel(state.loginRole)}</p>
            <h2>Sign in</h2>
          </div>
          ${isStaff ? `
  ...
  return `
    ...
    <main class="shell driver-shell">
      <section class="view-head">
        <div class="view-title">
          <p class="eyebrow">${brand().name} · Driver</p>
...
const kpiView = () => {
  ...
  return `
    ...
    <main class="shell driver-shell">
      <section class="view-head">
        <div class="view-title">
          <p class="eyebrow">${brand().name} · KPI</p>
          <h2>Fill KPI</h2>
...
const supervisorDashboard = () => {
  ...
  return `
    ...
    <main class="shell">
      <section class="view-head">
        <div class="view-title">
          <p class="eyebrow">${brand().name} · Supervisor</p>
          <h2>Daily metrics</h2>
...
const adminView = () => `
  ...
  <main class="shell">
    <section class="view-head">
      <div class="view-title">
        <p class="eyebrow">${brand().name} · Admin</p>
    ...
    <section class="stack">
      ...
      ${state.adminTab === 'overview' ? adminOverview() : state.adminTab === 'users' ? adminUsers() : state.adminTab === 'data' ? adminData() : adminSettings()}
    </section>
  </main>
`
...
const adminOverview = () => {
  ...
  const m = dashboardMetrics(today)
  const yest = dashboardMetrics(dayISO(-1))
  const subs = state.data.kpiSubmissions
  const users = state.data.users

  // Workforce
  const rosterDrivers = drivers()
  const activeSupervisors = supervisors()
  const admins = users.filter(u => u.role === 'admin' && u.status === 'active')
  const inactive = users.filter(u => u.status !== 'active')
  const activeRoutes = state.data.routes.filter(r => r.status === 'active')

  // Today's operations
  const submissionRate = rosterDrivers.length ? Math.round((m.driversSubmitted / rosterDrivers.length) * 100) : 0
  const incidentsToday = m.filtered.filter(k => String(k.incidents || '').trim()).length
  const radioToday = m.filtered.filter(k => String(k.radioRoomNotified || '').toLowerCase() === 'yes').length
  const offroadToday = m.filtered.filter(k => k.offroadDuties).length

  // Action needed
  const pendingHolidays = state.data.holidayRequests.filter(r => r.status === 'pending').length
  const openAlerts = state.data.alerts.filter(a => a.status === 'pending').length
  const pendingSick = state.data.sickLeaves.filter(s => s.status === 'pending').length
  const actionItems = pendingHolidays + openAlerts + pendingSick + incidentsToday

  // Last 7 days + all-time
  const week = comparisonRows(7)
  const weekParcels = week.reduce((s, r) => s + r.totalParcels, 0)
  const weekStops = week.reduce((s, r) => s + r.totalStops, 0)
  const weekHours = week.reduce((s, r) => s + r.totalHours, 0)
  const busiest = [...week].sort((a, b) => b.totalParcels - a.totalParcels)[0]
  const activeDays = new Set(subs.map(k => k.date)).size
  const allParcels = subs.reduce((s, k) => s + Number(k.parcels || 0), 0)
  const allStops = subs.reduce((s, k) => s + Number(k.stops || 0), 0)
  const avgDailyParcels = activeDays ? Math.round(allParcels / activeDays) : 0

  return `
    <section class="stack">
      <section class="panel panel-pad stack">
        <div class="view-title"><h3>Today · ${h(brisbaneDate())}</h3><p>Live depot operations for ${h(today)}.</p></div>
        <div class="stat-grid">
          ${stat('Drivers active today', m.driversSubmitted, `of ${rosterDrivers.length} on roster`)}
          ${stat('Pending check-ins', m.driversPending, m.driversPending ? 'No KPI submitted yet' : 'All drivers in')}
          ${stat('Submission rate', `${submissionRate}%`)}
          ${stat('Parcels today', m.totalParcels, trendDelta(m.totalParcels, yest.totalParcels))}
          ${stat('Stops today', m.totalStops, trendDelta(m.totalStops, yest.totalStops))}
          ${stat('Hours logged today', m.totalHours.toFixed(1))}
          ${stat('Incidents today', incidentsToday)}
          ${stat('Radio Room breaks', radioToday)}
          ${stat('Off-road duties', offroadToday)}
          ${stat('Tolls today', formatMoney(m.totalTolls))}
        </div>
      </section>

      <section class="panel panel-pad stack">
        <div class="view-title"><h3>Workforce</h3><p>Accounts and routes on the system.</p></div>
        <div class="stat-grid">
          ${stat('Active drivers', rosterDrivers.length, 'On roster')}
          ${stat('Supervisors', activeSupervisors.length)}
          ${stat('Admins', admins.length)}
          ${stat('Total accounts', users.length)}
          ${stat('Inactive accounts', inactive.length)}
          ${stat('Active routes', activeRoutes.length)}
        </div>
      </section>

      <section class="panel panel-pad stack">
        <div class="view-title"><h3>Action needed</h3><p>Items awaiting review or approval.</p></div>
        <div class="stat-grid">
          ${stat('Open items', actionItems, actionItems ? 'Needs attention' : 'All clear')}
          ${stat('Holiday requests', pendingHolidays, 'Pending approval')}
          ${stat('Radio Room alerts', openAlerts, 'Open')}
          ${stat('Sick leave', pendingSick, 'Pending')}
          ${stat('Incidents to review', incidentsToday, 'Today')}
        </div>
      </section>

      <section class="panel panel-pad stack">
        <div class="view-title"><h3>Last 7 days &amp; totals</h3><p>Rolling performance and lifetime totals.</p></div>
        <div class="stat-grid">
          ${stat('Parcels (7 days)', weekParcels)}
          ${stat('Stops (7 days)', weekStops)}
          ${stat('Hours (7 days)', weekHours.toFixed(1))}
          ${stat('Busiest day', busiest && busiest.totalParcels ? busiest.date : '—', busiest && busiest.totalParcels ? `${busiest.totalParcels} parcels` : 'No data yet')}
          ${stat('Avg parcels / active day', avgDailyParcels)}
          ${stat('KPI submissions', subs.length, 'All-time')}
          ${stat('Parcels (all-time)', allParcels)}
          ${stat('Stops (all-time)', allStops)}
        </div>
      </section>
    </section>
  `
...
const render = () => {
  ...
  app.innerHTML = `${html}${brandSwitch()}${state.toast ? `<div class="toast">${h(state.toast)}</div>` : ''}`
}
...
const exportJson = () => downloadFile(`depotos-export-${todayISO()}.json`, JSON.stringify(state.data, null, 2), 'application/json')
...
const exportCsv = () => {
  ...
  downloadFile('depotos-kpi.csv', csv, 'text/csv')
}
...
app.addEventListener('click', async (event) => {
  ...
  if (action === 'set-brand') {
    ...
    if (id && id !== getBrandId()) { setBrandId(id); applyBrand(); render() }
    return
  }
